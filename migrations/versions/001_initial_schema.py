"""Initial claims processing schema
...
def upgrade():
    op.execute("CREATE SCHEMA IF NOT EXISTS claims")
    ...
    op.execute("""
        CREATE TYPE document_type_enum AS ENUM (
            'lease', 'invoice', 'addendum', 'unknown', 'supporting_doc'
        )
    """)
    ...
    op.execute("""
        CREATE TYPE decision_status_enum AS ENUM ('approve', 'deny')
    """)
    
    op.execute("""
        CREATE TYPE decision_type_enum AS ENUM (
            'automated', 'manual_override', 'appeal', 'reprocessed'
        )
    """)
    
    op.execute("""
        CREATE TYPE audit_action_enum AS ENUM (
            'created', 'overridden', 'reviewed', 'appealed', 'superseded'
        )
    """)
    ...
    # Create claims table (partitioned)
    op.execute("""
        CREATE TABLE claims (
            id BIGSERIAL,
    ...
    # Create yearly partitions for claims
    for year in range(2020, 2028):
        op.execute(f"""
            CREATE TABLE claims_{year} PARTITION OF claims
    ...
    # Create decision_validation table
    op.execute("""
    ...
    # Create decision_audit_log table (partitioned)
    op.execute("""
    ...
    # Create monthly partitions for decision_audit_log
    for year in [2024, 2025]:
        for month in range(1, 13):
            ...
            op.execute(f"""
                CREATE TABLE decision_audit_log_{month_str} PARTITION OF decision_audit_log
                    FOR VALUES FROM ('{year}-{month:02d}-01') TO ('{next_year}-{next_month:02d}-01')
            """)
    
    op.execute("""
        CREATE TABLE decision_audit_log_default PARTITION OF decision_audit_log DEFAULT
    """)
    ...
    # Create indexes
    op.execute("CREATE UNIQUE INDEX idx_claims_tracking_number ON claims (claim_tracking_number)")
    op.execute("CREATE INDEX idx_claims_status_created_at ON claims (status, created_at DESC)")
    op.execute("CREATE INDEX idx_claims_claim_date ON claims (claim_date)")
    op.execute("CREATE INDEX idx_claims_policyholder_id ON claims (policyholder_id)")
    op.execute("CREATE INDEX idx_claims_property_id ON claims (property_id)")
    op.execute("CREATE INDEX idx_claims_priority_status ON claims (priority DESC, status) WHERE status IN ('pending', 'processing')")
    
    op.execute("CREATE INDEX idx_claim_documents_claim_id_type ON claim_documents (claim_id, document_type)")
    op.execute("CREATE INDEX idx_claim_documents_file_hash ON claim_documents (file_hash)")
    op.execute("CREATE INDEX idx_claim_documents_processing_status ON claim_documents (processing_status) WHERE processing_status = 'pending'")
    op.execute("CREATE INDEX idx_claim_documents_extracted_text_tsv ON claim_documents USING GIST (extracted_text_tsv)")
    
    op.execute("CREATE INDEX idx_decisions_claim_id_active ON decisions (claim_id, is_active) WHERE is_active = TRUE")
    op.execute("CREATE INDEX idx_decisions_engine_version_decided_at ON decisions (engine_version, decided_at DESC)")
    op.execute("CREATE INDEX idx_decisions_decided_at ON decisions (decided_at DESC)")
    op.execute("CREATE INDEX idx_decisions_decision_type ON decisions (decision_type)")
    op.execute("CREATE INDEX idx_decisions_superseded_by ON decisions (superseded_by) WHERE superseded_by IS NOT NULL")
    
    op.execute("CREATE INDEX idx_decision_validation_claim_id ON decision_validation (claim_id)")
    op.execute("CREATE INDEX idx_decision_validation_decision_id ON decision_validation (decision_id) WHERE decision_id IS NOT NULL")
    op.execute("CREATE INDEX idx_decision_validation_actual_decision_date ON decision_validation (actual_decision_date DESC)")
    
    op.execute("CREATE INDEX idx_decision_audit_log_claim_id_created_at ON decision_audit_log (claim_id, created_at DESC)")
    op.execute("CREATE INDEX idx_decision_audit_log_decision_id ON decision_audit_log (decision_id) WHERE decision_id IS NOT NULL")
    op.execute("CREATE INDEX idx_decision_audit_log_user_id ON decision_audit_log (user_id, created_at DESC)")
    op.execute("CREATE INDEX idx_decision_audit_log_action ON decision_audit_log (action, created_at DESC)")
    
    op.execute("CREATE INDEX idx_processing_queue_status_priority_scheduled ON processing_queue (status, priority DESC, scheduled_at) WHERE status IN ('pending', 'processing')")
    op.execute("CREATE INDEX idx_processing_queue_batch_id ON processing_queue (batch_id)")
    op.execute("CREATE INDEX idx_processing_queue_claim_id ON processing_queue (claim_id)")
    op.execute("CREATE INDEX idx_processing_queue_worker_id ON processing_queue (worker_id) WHERE worker_id IS NOT NULL")
...
def downgrade():
    # Drop triggers
    op.execute("DROP TRIGGER IF EXISTS trigger_decisions_audit_log ON decisions")
    op.execute("DROP TRIGGER IF EXISTS trigger_decisions_supersede ON decisions")
    op.execute("DROP TRIGGER IF EXISTS trigger_claim_documents_tsv ON claim_documents")
    op.execute("DROP TRIGGER IF EXISTS trigger_claims_updated_at ON claims")
    
    # Drop functions
    op.execute("DROP FUNCTION IF EXISTS log_decision_audit()")
    op.execute("DROP FUNCTION IF EXISTS supersede_old_decisions()")
    op.execute("DROP FUNCTION IF EXISTS update_extracted_text_tsv()")
    op.execute("DROP FUNCTION IF EXISTS update_updated_at_column()")
    op.execute("DROP FUNCTION IF EXISTS archive_old_claims(DATE)")
    op.execute("DROP FUNCTION IF EXISTS supersede_decision(BIGINT, BIGINT)")
    op.execute("DROP FUNCTION IF EXISTS calculate_decision_accuracy(DATE, DATE)")
    op.execute("DROP FUNCTION IF EXISTS get_active_decision(BIGINT)")
    
    # Drop tables
    op.execute("DROP TABLE IF EXISTS processing_queue CASCADE")
    op.execute("DROP TABLE IF EXISTS decision_audit_log CASCADE")
    op.execute("DROP TABLE IF EXISTS rules_changelog CASCADE")
    op.execute("DROP TABLE IF EXISTS decision_validation CASCADE")
    op.execute("DROP TABLE IF EXISTS decisions CASCADE")
    op.execute("DROP TABLE IF EXISTS claim_documents CASCADE")
    op.execute("DROP TABLE IF EXISTS claims CASCADE")
    
    # Drop enum types
    op.execute("DROP TYPE IF EXISTS queue_status_enum CASCADE")
    ...
    op.execute("DROP TYPE IF EXISTS claim_status_enum CASCADE")
